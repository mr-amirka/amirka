...
module.exports = (doc, prefix) => {
  ...
  return (styles) => {
    head = head || doc.head;
    if (!head) return;
    let trash = last, slot, node, item, revision, p, name, l = styles.length, i = 0; // eslint-disable-line
    last = {};
    while (i < l) {
      item = styles[i++];
      name = item.name;
    ...
    for (name in trash) (p = (node = trash[name][0]).parent) && p.removeChild(node); // eslint-disable-line
  };
};
