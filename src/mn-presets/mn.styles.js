...
module.exports = (mn) => {
  ...
  forIn({
    ...
    [ '', 'min', 'max' ].forEach((sfx) => {
      ...
      mn(essencePrefix + sfx, p => {
        ...
        const sz = (camel ? camel.toLowerCase() : (num ? (num + unit) : '100%')) + p.i;
        const style = {};
  ...
  */
  mn('fw', p => {
    ...
    return {
      style: {
        fontWeight: camel ? camelToKebabCase(lowerFirst(camel)) : (100 * intval(p.num, 1, 1, 9))
      }
    };
  });
  ...
  */
  ...
  ['x', 'y', 'z'].forEach((suffix) => {
    ...
    mn('r' + suffix, p => ({
      style: {
        transform: prefix + (p.value || '180') + (p.unit || 'deg') + ')'
      }
  	}));
  });
  ...
  */
  ...
  (() => {
    ...
    forIn({
      ...
      mn(essenceName, p => {
        const style = {};
        style[propName] = lowerFirst((p.suffix || '')
  ...
  */
  ...
  forIn({
    ...
    mn(essenceName, p => {
      ...
      if (!suffix) return;
      const style = {};
      style[opt[0]] = camelToKebabCase(lowerFirst(suffix)) + p.i;
      return {
        style,
        priority: opt[1]
      };
  ...
  forIn({
    ...
    mn(essenceName, p => {
      ...
      if (!suffix) return;
      const style = {};
      style[propName] = camelToKebabCase(lowerFirst(suffix)) + p.i;
      return { style };
    });
  });
